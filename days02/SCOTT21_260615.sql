...
--1.Structured Query ==구조화 된 질의 언어
--
--클라이언트 도구                                오라클서버
--sqlplus.exe                                 질의/ 응답
...
--                                            SQL
--2. SQL대상 : 테이블 또는 뷰
--3. PL/SQL  :절차적인 언어 +SQL
-- 오라클
-- 
--4.SQL 5가지종류
-- 
-- 1)DQL - 조회 ( SELECT)
-- 2)DML - 조작 (INSERT문, UPDATE,DELEATE문) RENAME,TRUNCATE
-- 3)DDL - 정의 (CREAT문, DROP문, ALTER문)
-- 4)DCL - 권한 (GRANT문, REVOKE문)
-- 5)TCL - 트랜잭션

--SCOTT --
-- 1) SCOTT계정이 소유하고 있는 모든 테이블 정보를 확인하는 쿼리를 작성
...
--FROM user_tables; -- scott소유의 테이블정보


--SELECT문
-- 대상 : 테이블또는 뷰
-- SELECT 문 : 7개의 절(clause) (암기)
--   1>6>2>3>>4>5>7   << 작성순서
--   1. WITH
--   2. FROM       필수
--   3. WHERE       
...
--SELECT emp.*
SELECT e.*
FROM scott.emp e; -- 별칭 (alias);



-- 모든 부서정보를 조회(SELECT문 사용)
SELECT *
FROM scott.dept;
...
-- DISTINCT 키워드 : 중복제거 하는 함수
SELECT DISTINCT job,ename


SELECT job,ename -- 아무것도 안적으면 ALL 생략됨
...
-- 각사원들이 속해 있는 부서번호를 조회
SELECT deptno
   --ename, deptno
FROM emp;


-- 사원정보를 조회(사원번호, 사원명, 입사일자)
-- sal + comm = 한달 월글( pay)
SELECT *
FROM emp;


-- NULL값을 다른값으로 대체 nvL(comm,0)>>comm의 null값을 0으로
SELECT empno, ename, hiredate,sal,comm,nvL(comm,0)
    -- ,sal+NVL(comm,0) AS "PAY"--괄로안에 별칭
    ,sal+NVL2(comm,comm,0) PAY --AS,""(""사이에 공백이 있는경우사용!!) 생략가능 
FROM emp;
...
SELECT '이름은 '''|| ename || ' ''이고 ' || ' 잡은 ''' || job || ''' 입니다' TEXT
FROM emp;



-- NULL 처리 함수 :  NVL(), NVL2()
-- [문제] 사원테이블에서 직속상사가 없는 사원은 'CEO'fkrh cnffur
-- (사원번호 , 사원명, 직속상사) 출력 숫자열을 문자열로 변환
--SELECT empno,ename,NVL2(mgr,TO_CHAR(mgr), 'CEO') AS 직속상사
--FROM emp;


-- emp 테이블에서 사원번호, 사원명 , 입사일자 , 부서번호 조회
-- 조건 부서번호가 10번인 사원들만 조회.
SELECT *
FROM emp;
...
Plan hash value: 3956160932   -- 실행계획에 고유 식별자...


실행순서     수행작업        객체명   예상결과             비용           옵티마이저
식별자(ID)                 인덱스 등 행의수 데이터크기  CPU사용량 + IO사용량  예상시간
--------------------------------------------------------------------------
| Id  | Operation         | Name | Rows  | Bytes | Cost (%CPU)| Time     |
--------------------------------------------------------------------------
|   0 | SELECT STATEMENT  |      |     1 |    42 |     3   (0)| 00:00:01 |
|*  1 |  TABLE ACCESS FULL| EMP  |     1 |    42 |     3   (0)| 00:00:01 |
--------------------------------------------------------------------------
 
 TABLE ACCESS FULL : 테이블의 전체 스캔 ~~
...
-- [문제] emp 테이블에서 10번부서번호가 아닌 사원번호, 사원명 , 입사일자 , 부서번호 조회
   
SELECT empno,ename,hiredate,deptno
FROM emp
WHERE deptno IN (20,30,40);  --sql 연산자
--WHERE NOT (deptno = 10);
...
--city 컬럼 : 사원의 출신지역
-- [문제] insa 테이블에서 수도권출신 사원들이 몇명인지 조회하는 쿼리르 작성하세요
SELECT COUNT (city)
FROM insa
--WHERE city NOT IN('서울','경기','인천') -- 수도권이 아닌
--WHERE city IN('서울','경기','인천')
WHERE NOT(city = '서울' OR  city = '경기'  OR city = '인천')
--WHERE city = '서울' OR  city = '경기'  OR city = '인천'
--WHERE city != '서울' OR  city != '경기'  OR city != '인천'  -- 수도권이 아닌
ORDER BY city ASC;-- Asc 오름차순 DESC 내림차순


-- [문제] emp 테이블에서 com이 NULL 인 사원의 정보를 모두 조회(검색)
...
SELECT empno,ename,job,sal,sal+comm pay
FROM emp
WHERE sal+ncomm>=2000 AND sal+comm<=4000;
