...
INSERT INTO emp_sal VALUES(7369, 800);
INSERT INTO emp_sal VALUES(7499, 1600);

INSERT INTO emp_new VALUES(7369, 1000);
...
USING emp_new s
ON(t.empno = s.empno)
WHEN MATCHED THEN      -- 일치할때
    UPDATE SET t.sal =  s.sal
WHEN NOT MATCHED THEN  --일치하지 않을때
...
-- DELETE도 가능하다 : 급여가 2000이하가 되면 삭제
...
WHEN MATCHED THEN
    UPDATE                  -- 일치하는게 있다면 업데이트부터
       SET t.sal = s.sal
    DELETE                  -- s.sal <= 2000 조건을 만족한다면 삭제
     WHERE s.sal <= 2000;
...
-- 한 문장으로 처리 가능하며, 데이터 동기화 작업(ETL, DW, 배치 프로그램)에서 매우 많이 사용됩니다.


------------------------------------------------------------------------------------------
-- 제약조건
-- 1) 제약 조건 확인 :  user_constraints 뷰(View) 확인가능

SELECT *
FROM user_constraints
WHERE table_name = 'EMP';

-- 2) 제약 조건 정의 : 데이터 무결성(정확성, 일관성)을 보장하기 위해 설정하는 규칙.

-- 3) CREATE TABLE 문: 제약조건을 설정하는 2가지 방법
...
INSERT INTO tbl_constraint (empno,ename,deptno,kor,email,city) VALUES(1111,'홍길동',null,null,null,null);
INSERT INTO tbl_constraint (empno,ename,deptno,kor,email,city) VALUES(1111,'서무식',null,null,null,null);
...
-- 데이터의 무결성을 보장하는 규칙 : 제약조건을 설정...
DROP TABLE tbl_constraint PURGE;

-- 다시생성
CREATE TABLE tbl_constraint(
-- 컬럼명    타입(크기)       [ CONSTRAINT 제약조건명 ] 생략하면 자동생성
    empno   NUMBER(4)     NOT NULL CONSTRAINT PK_tblconstraint_empno PRIMARY KEY
    ,ename  VARCHAR2(20)  NOT NULL
    --dept(deptno참조키) -> tbl_constraint(deptno외래키Fk)
    ,deptno NUMBER(2)     CONSTRAINT FK_tblconstraint_empno REFERENCES dept(deptno)  -- ON DELETE CASCADE | ON DELETE SET NULL
...
-- empno (PK)  : NULL
-- SQL 오류: ORA-01400: NULL을 ("SCOTT"."TBL_CONSTRAINT"."EMPNO") 안에 삽입할 수 없습니다
INSERT INTO tbl_constraint (empno, ename, deptno, kor, email, city )
VALUES ( null, 'kim', 10, 90, 'kim@naver.com', '서울');

-- dept(deptno) 참조
-- ORA-02291: 무결성 제약조건(SCOTT.FK_TBLCONSTRAINT_EMPNO)이 위배되었습니다- 부모 키가 없습니다
INSERT INTO tbl_constraint (empno, ename, deptno, kor, email, city )
VALUES ( 1001, 'kim', 90, 90, 'kim@naver.com', '서울');


-- ORA-02290: 체크 제약조건(SCOTT.CK_TBLCONSTRAINT_CITY)이 위배되었습니다
INSERT INTO tbl_constraint (empno, ename, deptno, kor, email, city )
VALUES ( 1001, 'kim', 10, 90, 'kim@naver.com', '대전');

-- ORA-02290: 체크 제약조건(SCOTT.CK_TBLCONSTRAINT_KOR)이 위배되었습니다
INSERT INTO tbl_constraint (empno, ename, deptno, kor, email, city )
VALUES ( 1001, 'kim', 10, 190, 'kim@naver.com', '서울');

-- INDEX : PK(empno), UK(email) 인덱스 생성된다. (암기)
SELECT *
FROM user_indexes
...
-- 테이블 삭제 후에 복사 + 붙이기 : 테이블 레벨 방식으로 제약조건 설정
DROP TABLE tbl_constraint PURGE;
CREATE TABLE tbl_constraint(
    ...
    ,CONSTRAINT FK_tblconstraint_empno FOREIGN KEY(deptno)  REFERENCES dept(deptno)  -- ON DELETE CASCADE | ON DELETE SET NULL
...
-- NOT NULL 제약조건을 제외한 그외 제약 조건은 복사되지 않는다.
-- 1) emp , dept 제약조건 확인
SELECT *
FROM user_constraints
WHERE table_name IN('TBL_EMP','TBL_DEPT');

-- 2) tbl_emp , tbl_dept 기존 테이블에 제약조건 추가.
...
-- ORA-02291: 무결성 제약조건(SCOTT.FK_TBLEMP_DEPTNO)이 위배되었습니다- 부모 키가 없습니다
--INSERT INTO tbl_emp (empno, deptno) VALUES(9999, 90);

-- ORA-02292: 무결성 제약조건(SCOTT.FK_TBLEMP_DEPTNO)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- 예) FK제약조건을 삭제, FK 제약조건 추가 + ON DELETE 옵션
ALTER TABLE tbl_emp
...
--4.1.1 규칙1: 모든 개체는 릴레이션으로 변환한다
--4.1.2 규칙2: 다대다(n:m) 관계는 릴레이션으로 변환한다
...
--    ㄴ. 함수적 종속성 : 속성들간의 관련성
--        예) emp사원테이블
--        empno(PK) ename, sal, hidedate,job등등
--        sal 은 empno 에 함수적으로 종속이 된다.
--        X -> Y
--    결정자   종속자
-- 예) ename  empno
-- ename(종속자) 은 empno(결정자) 에 함수적으로 종속이 된다.
--        Y     X
--        X ->  Y
-- (1) 완전 함수적 종속
--        여러개의 속성이 모여 하나의 기본키가 될때  == 복합키
--        [ 급여지급일  + 사원번호] 급여액 컬럼
